...
//column choices
const firstColumn = function (x) {return x <= 100}
const secondColumn = function (x) { return x >= 101 && x <= 200}
const thirdColumn = function (x) { return x >= 201 && x <= 300}
const fourthColumn = function (x) { return x >= 301 && x <= 400}
const fifthColumn = function (x) { return x >= 401 }
let currentColumn = function (objectWithXCoordinate) {
  let result = ''
  if (objectWithXCoordinate.x >= 4 && objectWithXCoordinate.x <= 101) {
    result = 'firstColumn'
  } else if (objectWithXCoordinate.x >= 102 && objectWithXCoordinate.x <= 200) {
    result = 'secondColumn'
  } else if (objectWithXCoordinate.x >= 201 && objectWithXCoordinate.x <= 298) {
    result = 'thirdColumn'
  } else if (objectWithXCoordinate.x >= 300 && objectWithXCoordinate.x <= 398) {
    result = 'fourthColumn'
  } else {
    result = 'fifthColumn'
  }
...
//movement
  handleInput (input) {
    ...
    if (this.x === stone.x && this.y === stone.y) {
      ...
      let currentSpriteIndex = stoneSpriteArray.findIndex(k => k === stone.sprite);
      let lastSprite = stone.sprite === stoneSpriteArray[stoneSpriteArray.length -1];
      stone.sprite =
          lastSprite ? stoneSpriteArray : stoneSpriteArray[currentSpriteIndex + 1]
    }
  }
