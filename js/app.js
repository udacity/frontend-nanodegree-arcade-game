...
Enemy.prototype.update = function (dt) {
  ...
  this.pos.x = (newX < gameSize.width) ? newX : 0 - this.size.width;
};
...
Player.prototype.died = function () {
  ...
  // on regular death, just reset to start position
  this.setStartPosition();

  // reset flag so player can move again (flag got set in collision detection)
  this.justDied = false;
};
...
Player.prototype.handleInput = function (keycode) {
  ...
  if (this.justScored === true || this.justDied === true || difficulty.state === true) {
    return;
  }
...
function setDifficulty(choice) {
  ...
  // Set the state to true to halt input and display a Resetting... message
  difficulty.state = true;

  // and save the choice so we don't reset if they click the same button again
  ...
  // allow for some time to display a reset message so the user knows
  // we're responding to their setting choice
  setTimeout(function () { gameReset(); }, 2000);
}

// this allows for the pause to show Game Over on screen
// before restarting
function gameOver() {
...
// On page refresh, the radio buttons can be out of sync with the default game settings
// Use this to reset to defaults
function setRadioButtons() {
  var difficultyButtons = document.getElementById('difficultyButtons');
  for (var i = 0; i < difficultyButtons.length; i++) {
    difficultyButtons[i].checked = (difficultyButtons[i].id === 'medium') ? true : false;
  }
}
// *********************************************************************
// Here we create our enemies on first run of the game
// As well as our player
