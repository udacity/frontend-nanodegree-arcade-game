...
Engine.prototype.updatePlayer = function() {
    if(this.parts['player'].itRestarted()) {
        this.parts['player'].init(
            this.parts['resources'].urlImage('characters', 'boy'),
            this.parts['scenario'].getStartPoint()
        );
    }
    this.parts['player'].update();
};
...
Engine.prototype.updateEnemies = function(dt) {
	...
	this.enemies.forEach(function(enemy) {
		if (enemy.itRestarted()) {
            var environments, route;

			this.parts['traffic'].removeOfRoute(enemy.lastTraveledRoute);
            environments = enemy.getEnvironments(),
